...
def verify():
	if request.method == 'POST':
		...
		print(anchor_img_file)

		A = Image.frombytes(data=anchor_img_file, size=(225, 155), mode='L')
		...
		if dist <= 0.16189:  # Threshold obtained using Test.py
			return render_template('verify.html', result="<h6>Signatures are the same</h6>")
		else:
			return render_template('verify.html', result="<h6>Signatures are the different</h6>")
	else:
